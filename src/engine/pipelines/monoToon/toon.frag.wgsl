...
fn main(input: Input) -> @location(0) vec4f
{
  ...
  let c_spec = (s_spec * fs_uni.material.m_spec) * smoothstep(0.005, 0.01, pow(max(dot(v, r), 0), fs_uni.material.m_gls * fs_uni.material.m_gls));
  let c_diff = (s_diff * fs_uni.material.m_diff) * smoothstep(0.0, 0.01, dot(n, l));
